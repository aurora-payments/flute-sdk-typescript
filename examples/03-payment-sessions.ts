/**
 * Example: server-side bootstrap for a Flute Elements / Checkout flow.
 *
 * Maps to PRD §FR-3.3 (Payment Sessions). The merchant's backend creates
 * a payment session, hands the session id to its front-end, and the
 * customer completes the payment via Flute Elements / Checkout. The
 * back-end then polls or receives a webhook to confirm.
 *
 * Run with:
 *
