# AGENTS.md — using `@flute-payments/sdk` from an AI agent
...
code that imports `@flute-payments/sdk` and runs against the Flute
...
```bash
npm install @flute-payments/sdk@^0.1.3
```

```ts
import { Flute, FluteApiError, Environment } from '@flute-payments/sdk';
...
Credentials are read from env vars; never embed them in source. The SDK
caches the OAuth bearer token in-process and refreshes transparently.
Every error thrown extends `FluteError` — branch on the subclass first,
then on `httpStatus` for retry decisions.

## Version requirement

| Range     | Status                           | Notes                                                                                                                                                                                                                                                                  |
| --------- | -------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `< 0.1.3` | **Broken on default `baseUrls`** | Versions `0.1.0` → `0.1.2` shipped a `/isv-api` prefix on the ISV BFF host that does not exist on the live API. Every call through `transactions.*` and `settings.*` returns `404 Not Found`. OAuth and `paymentSessions.*` were unaffected. Do not pin below `0.1.3`. |
| `^0.1.3`  | Recommended                      | Bug-fixed defaults. Resource calls reach UAT and production successfully.                                                                                                                                                                                              |
...
npm install @flute-payments/sdk@0.1.3 --save-exact
```
...
`aurora-payments/flute-sdk-typescript@release.yml`.
...
```ts
import type { TokenStorage, StoredToken } from '@flute-payments/sdk';
...
Every error thrown by the SDK extends `FluteError`. Discriminate on the
subclass first:
...
```ts
import { verifyWebhookSignature, FluteWebhookError } from '@flute-payments/sdk';
...
Sandbox and production credentials are **separate**. Provisioning each
pair is the operator's responsibility; the agent never creates them.

The SDK enforces HTTPS on every base URL except `localhost`,
`127.0.0.1`, and `[::1]`, which are accepted over plain HTTP for local
development against an Aspire stack. Per-environment overrides are
accepted under `baseUrls`:
...
`baseAmount` is in the merchant's smallest currency unit (`1000` = $10.00 USD). Always pass `currencyCode` explicitly even though the OpenAPI spec marks it optional — the live API returns HTTP 500 if it is omitted.

## Things to avoid

- **Don't pin below `0.1.3`.** `0.1.0` → `0.1.2` are broken on default `baseUrls` for the entire `transactions.*` and `settings.*` surface; the bug is documented in `aurora-payments/flute-sdk-typescript#19`.
- **Don't omit `currencyCode` in `calculateAmount`.** Despite the OpenAPI spec marking it optional with a `"USD"` default, the live backend returns HTTP 500 (`Validation failed: CurrencyId must be greater than 0`) if it is missing. Always pass `currencyCode: 'USD'` (or the merchant's currency) explicitly.
- **Don't construct multiple `Flute` instances per process** unless you need different credentials. The OAuth token cache is per-instance; spawning N clients fans out to N concurrent token fetches and lets them race against the gateway's rate limit.
- **Don't ignore `FluteRateLimitError`.** `retryOn429` is `false` by default to keep the SDK predictable. The agent owns the backoff policy. Honor `err.retryAfterMs` if present.
- **Don't reuse the same `idempotencyKey` for different request bodies.** The server returns `FluteIdempotencyError` (HTTP 409) when the key is recognized but the body differs. Either replay the original body verbatim or pick a fresh key.
- **Don't re-serialise the webhook body before verifying.** Capture the raw bytes the framework received (Express: `req.rawBody` via `body-parser`'s `verify` callback; Fastify: `request.body` is the parsed object — set up a content-type parser to keep the raw stream).
- **Don't share `MemoryTokenStorage` across processes.** It is in-process only. Use a Redis / KV-backed `TokenStorage` for multi-replica deployments so cold starts and concurrent replicas do not all re-authenticate.
- **Don't log `clientSecret`, `accessToken`, or webhook secrets.** The SDK redacts these in its own debug output; agent-side logging is the agent's responsibility.
- **Don't disable HTTPS on production.** The SDK enforces it on non-loopback URLs by design (see PRD §NFR-1).

## See also

- [`README.md`](README.md) — human-readable overview, install + quickstart, 5 ISV flow recipes, environment guidance, test cards.
- [`src/index.ts`](src/index.ts) — public API entry point. Anything not re-exported there is internal.
- [`openapi/isv-api-v2.json`](openapi/isv-api-v2.json) — wire-format source of truth (spec the type generator consumes).
- [`src/types/generated/isv-api-v2.d.ts`](src/types/generated/isv-api-v2.d.ts) — generated DTO types. Regenerate with `npm run openapi:types`.
- [`src/errors.ts`](src/errors.ts) — full error class hierarchy with constructor signatures.
- [`CHANGELOG.md`](CHANGELOG.md) — release history; bug fixes and breaking changes are flagged here.
